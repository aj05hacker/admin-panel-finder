...
class ajadminfinder:
    ...
    def credit(self):
        ...
        print(
            ...
            f"""
                           _       _         ___ _           _
                          | |     (_)       / __(_)         | |
                 _____  __| |____  _ ____ _| |__ _ ____   __| |_____  ____
           Aj's (____ |/ _  |    \| |  _ (_   __| |  _ \ / _  | ___ |/ ___)
                / ___ ( (_| | | | | | | | || |  | | | | ( (_| | ____| |
                \_____|\____|_|_|_|_|_| |_||_|  |_|_| |_|\____|_____|_|
            💚 version {meta.__version__} created by abdul hajees 💚
        ...
        yield
        print(green, """\tThank you for using AJ-adminFinder""", RESET_ALL)
        print(
    ...
    def create_link(website):
        ...
        if url.host[0:4] == "www.":
            ...
            for n in ajadminfinder.get_links():
    ...
    def proxy(self, prox, headers):
        try:
            proxies = ajadminfinder.get_proxy(prox)
            with httpx.Client(headers=headers, proxies=proxies) as client:
    ...
    async def url(self, website, headers, proxies):
        try:
            if ajadminfinder.check_url(website, headers, proxies):
                print(
                    magenta,
                    BOLD,
                    website,
                ...
                exit(1)
            urls = ajadminfinder.create_link(website)
            admin_count = 0
